...
export class EmailService {
  ...
  // Modifier le constructeur pour une meilleure compatibilité
constructor(
  ...
  if (!user || !pass) {
    console.warn('⚠️  Credentials manquants - Service email désactivé');
    return;
  }
  ...
  try {
    this.transporter = nodemailer.createTransport({
      ...
      host: 'smtp.gmail.com',
      port: 465, // Port SSL plus fiable sur certains hébergeurs
      secure: true, // true pour 465
      auth: {
        user: this.configService.get('EMAIL_USER'),
        pass: this.configService.get('EMAIL_PASS'),
      },
      // Configuration réseau plus permissive
  ...
  async onModuleInit() {
    ...
    try {
      ...
      // Vérifier si c'est un timeout (port bloqué)
      if (
        err.message.includes('timeout') ||
        err.message.includes('ECONNREFUSED')
      ) {
        console.error('🚫 SMTP probablement bloqué par Render gratuit');
      }
  ...
  async sendVerificationEmail(email: string, token: string, language: string) {
    if (!this.checkEmailService()) {
      console.log(
        `📧 Email de vérification non envoyé à ${email} (service désactivé)`,
      );
      return false;
    }
  ...
  async sendResetPasswordEmail(email: string, token: string, language: string) {
    if (!this.checkEmailService()) {
      console.log(
        `📧 Email de reset non envoyé à ${email} (service désactivé)`,
      );
    ...
    try {
      ...
      const subject = this.i18n.translate('common.FORGOT_PASSWORD_SUBJECT', {
        lang: language,
      });
      const body = this.i18n.translate('common.PASSWORD_RESET_BODY', {
        lang: language,
        args: { url },
      });
      const appName = this.configService.get('APP_NAME') || 'Go4Motors';
      ...
      throw new InternalServerErrorException(
        'Échec envoi email de réinitialisation',
      );
    }
  }
}
